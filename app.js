/* ═══════════════════════════════════════════
   FUERZA PRO — app.js
   Firebase Auth + Firestore + todas las funciones
═══════════════════════════════════════════ */
...
/* ── Estado global ── */
let currentUser     = null;  // username (ej: 'jhoao')
...
window.handleRegister = async function() {
  ...
  if (!/^[a-z0-9_]+$/.test(username)) { showErr(errEl,'⚠ Usuario: solo letras, números y guión bajo'); return; }
  ...
  if (snap.exists()) { showErr(errEl,'⚠ Ese nombre de usuario ya está en uso'); return; }

  try {
    const cred = await createUserWithEmailAndPassword(auth, email, pass);
    // Guardar username → uid y datos del usuario
...
/* ═══════════════════════════════════════════
   onAuthStateChanged — detecta login/logout automático
═══════════════════════════════════════════ */
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user.emailVerified) {
    // Si no verificó, mostramos aviso pero dejamos entrar igual
    // (puedes cambiar esto a return si quieres forzar verificación)
  }
  ...
  if (!snap.exists()) {
    // Usuario de Firebase Auth pero sin datos en Firestore (caso jhoao/karen migrados)
    // Intentar encontrar por username en colección antigua
  ...
  return `
    <div class="exercise-card" id="card-${ex.id}">
      ...
      <div class="card-actions">
        ...
        <button class="btn-weight" onclick="openWeightModal('${ex.id}',${targetUID?`'${targetUID}'`:'null'})" title="Peso">⚖️</button>
        ${adminBtns}
      </div>
    </div>`;
}
...
window.saveExercise = async function() {
  ...
  closeModal();
  if (targetUID===currentUID) { exercises=exs; currentDay=day; document.getElementById('sectionTitle').innerHTML=day.toUpperCase(); renderDaysNav(); renderExercises(); updateProgress(); }
  else adminLoadUser();
};
...
function renderDietContent() {
  ...
  if (!dayM.length) {
    cont.innerHTML=`<div class="empty-state"><div class="icon">🥗</div><p>No hay comidas para <strong>${currentDietDay}</strong>.</p>${isAdmin?`<button onclick="openMealModal('${currentDietDay}')">+ Agregar comida</button>`:''}</div>`; return;
  }
  let html='';
    ...
    html+=`<div class="meal-block">
      ...
      ${tm.map(m=>`<div class="meal-card">
        <div class="meal-card-body">
          ...
          <div class="card-meta">${m.cal?`<span class="badge badge-cal">${m.cal} kcal</span>`:''}${m.prot?`<span class="badge badge-prot">P:${m.prot}g</span>`:''}${m.carbs?`<span class="badge badge-carbs">C:${m.carbs}g</span>`:''}${m.fat?`<span class="badge badge-fat">G:${m.fat}g</span>`:''}</div>
...
window.deleteMeal = async function(id) {
  ...
  meals=meals.filter(m=>m.id!==id); await saveUser({meals});
};
window.saveMeal = async function() {
  ...
  const meal={id:editId||Date.now().toString(),time:document.getElementById('mealTime').value,day:document.getElementById('mealDay').value,name,qty:document.getElementById('mealQty').value.trim(),cal:document.getElementById('mealCal').value,prot:document.getElementById('mealProt').value,carbs:document.getElementById('mealCarbs').value,fat:document.getElementById('mealFat').value,notes:document.getElementById('mealNotes').value.trim()};
  if(editId){const i=meals.findIndex(m=>m.id===editId);if(i>=0)meals[i]=meal;}else meals.push(meal);
...
function renderBodyData(){
  ...
  hist.innerHTML=`<div class="stats-history-title">Historial</div>`+[...bodyData].reverse().map((r,i)=>`<div class="history-row"><div class="history-date">${formatDate(r.date)}</div><div class="history-values">${r.peso?`<span class="hv-badge">⚖️${r.peso}kg</span>`:''}${r.grasa?`<span class="hv-badge">🔥${r.grasa}%</span>`:''}${r.musculo?`<span class="hv-badge">💪${r.musculo}%</span>`:''}${r.bmi?`<span class="hv-badge">BMI ${r.bmi}</span>`:''}${r.visceral?`<span class="hv-badge">Visceral ${r.visceral} ${getSemaforo('visceral',r.visceral)}</span>`:''}</div><button class="btn-delete-stat" onclick="deleteBody(${bodyData.length-1-i})">🗑</button></div>`).join('');
}
...
async function renderAdmin(){
  ...
  const users=snap.docs.map(d=>({uid:d.id,username:d.data().username||d.id})).filter(u=>u.username!==ADMIN_USERNAME);
...
window.adminLoadUser=async function(){
  ...
  const d=snap.data(),exs=d.exercises||[];
...
function firebaseErrMsg(code){
  const msgs={'auth/email-already-in-use':'⚠ Ese correo ya está registrado','auth/invalid-email':'⚠ Correo inválido','auth/weak-password':'⚠ Contraseña muy débil (mínimo 6 caracteres)','auth/user-not-found':'⚠ No existe una cuenta con ese correo','auth/wrong-password':'⚠ Contraseña incorrecta','auth/invalid-credential':'⚠ Correo o contraseña incorrectos','auth/too-many-requests':'⚠ Demasiados intentos. Espera unos minutos.'};
